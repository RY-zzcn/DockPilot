...
Agent Docker 镜像包含 Docker CLI、Compose v2 插件、CA 证书和时区数据，这是它能够通过宿主机 `/var/run/docker.sock` 执行 `docker compose pull/up` 的原因。Agent 程序本身是单个 Go 二进制；如果只追求轻量、多架构和类似探针的一键接入体验，优先选择二进制 + systemd。

当本机已经存在 `dockpilot-server` 容器时，安装脚本会自动让 Docker Agent 加入 Server 所在网络，并把 Agent 连接地址改为 `http://dockpilot-server:8080`。这可以避免 Server 和 Agent 部署在同一台 VPS 时，容器内访问公网 IP、`127.0.0.1` 或错误 DNS 名称导致离线。
...
- `detect_updates` 任务会以 Compose 文件所在目录作为 project directory，执行 `docker compose --project-directory <dir> -f <compose.yml> config --images` 获取镜像列表，因此会正常读取同目录 `.env` 和相对路径。
...
- `automatic`：到点先做 Compose 配置预检，再执行 `docker compose pull --ignore-buildable` 和 `docker compose up -d --remove-orphans`。如果节点上的 Compose 版本不支持 `--ignore-buildable`，Agent 会自动重试普通 `docker compose pull`。任务日志会记录更新前后容器镜像 ID 是否变化，便于区分“已是最新”和“命令执行失败”。
